(() => {
  ...
  const updateImage = (selector, image) => {
    ...
    if (!img || !image) return;
    img.src = image.src;
    img.srcset = image.srcset || image.src;
    img.alt = image.alt;
  };

  const buildVehicleLink = (id) => new URL(`vehicle-detail.html?car=${encodeURIComponent(id)}`, window.location.href).href;

  const renderContactVehicle = () => {
